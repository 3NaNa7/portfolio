...
export default function ResumeHighlights() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        animate={reducedMotion ? {} : { y: [0, -4, 0], rotate: [-1.5, 1.5, -1.5] }}
      ...
      >
        ...
        >
          ...
          <circle cx="30" cy="25" r="3" fill="var(--color-coral)" />
        </svg>
      </motion.div>
      ...
      <div className="flex flex-wrap justify-center gap-3 max-w-lg mt-2">
        {resumeHighlights.openTo.map((role) => {
          const rotation = deterministicVariance(role, -2, 2);
          return (
            <div
              key={role}
              style={{
                ...
                padding: '7px 16px',
                borderRadius: '999px',
                border: '1.5px solid rgba(44,110,104,0.22)',
                background: 'rgba(44,110,104,0.04)',
                color: 'var(--color-ink)',
                boxShadow: '0 2px 5px -1px rgba(0,0,0,0.02)',
      ...
      {/* 4. RESUME DOWNLOAD BUTTON (UNCHANGED logic & classes) */}
